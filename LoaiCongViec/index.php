...
<head>
    ...
    <!-- Our Custom CSS -->
    <link rel="stylesheet" href="style_LCV.css">
    ...
    <div class="wrapper">
        ...
        <nav id="sidebar">
            ...
            <ul class="list-unstyled components" style="margin-bottom: 0px; padding: 0px">
                ...
                <li><a href="index.php"><i class="fas fa-tasks"></i> Loại Công Việc</a></li>
        ...
        <div id="content">
            ...
            <div class="content-header">
                <div class="container-fluid">
                    <div class="row mb-2">
                        <div class="col-sm-6">
                            <h3 class="m-0">Quản Lý Loại Công Việc</h3>
                        </div>
                    </div>
                    ...
                    <div class="col-lg-12">
                        <div class="card card-outline card-success" style="border-top: 3px solid rgb(48, 162, 48); border-radius: 5px;">
                            <div class="card-header">
                                ...
                                <div class="row">
                                    <form method="post">
                                        <input type="text" name="txtSearch" style="width: 450px; margin-left:10px" placeholder="Tìm Kiếm Theo Loại Công Việc">
                                    </form>
                                    <!-- button thêm -->
                                    <div class="card-tools" style="margin-left: 400px">
                                        <a class="btn btn-block btn-sm btn-default btn-flat border-primary" href="them_lcv.php" ><i class="fa fa-plus"></i> Thêm Loại Công Việc</a>
                                    </div>
                                </div>
                            </div>
                            <table class="table-bordered">
                                <thead>
                                    <tr>
                                        ...
                                        <th scope="col" style="text-align: center">Tên Loại Công Việc</th>
                                        <th scope="col" style="width: 110px; text-align: center">Thao Tác</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <?php
                                    ...
                                    if ($conn != true) {
                                        ...
                                        $sql = mysqli_query($conn, "select * from `loaicv`");
                                        ...
                                        $sql = "SELECT * FROM `loaicv` LIMIT $start,$limit";

                                        //----------------------------------Tìm Kiếm + Phân Trang -----------------------------
                                        if (isset($_POST['txtSearch']) && $_POST['txtSearch'] != '') {
                                            $KeyWord = $_POST['txtSearch'];
                                            $KeyWor=$_POST['txtSearch'];
                                            $sql = mysqli_query($conn, "SELECT * FROM  loaicv WHERE TenLoaiCongViec LIKE N'%" . $KeyWord . "%'   
                                        or id like N'%" . $KeyWor . "%' "  );
                                            //b1:tính tổng các bản ghi
                                            $total = mysqli_num_rows($sql);
                                            $limit = 5;
                                            ...
                                            $total_page = ceil($total / $limit);
                                            //lấy trang hiện tại
                                            $cr_page = isset($_GET['page'])? $_GET['page'] : 1;//kiểm tra điều kiện đúng sai 
                                            $start = ($cr_page - 1) * $limit;
                                            $sql = "SELECT * FROM loaicv WHERE TenLoaiCongViec LIKE N'%" . $KeyWord . "%' or id like N'%" . $KeyWor . "%'  LIMIT $start,$limit";
                                        } else {
                                            //------------------------------phân trang--------------------------------
                                            $sql = mysqli_query($conn, "select * from `loaicv`");
                                            //b1:tính tổng các bản ghi
                                        ...
                                        if ($result) {
                                            while ($row = mysqli_fetch_assoc($result)) {
                                                $idtang++;
                                                $id = $row['id'];
                                                $tenlcv = $row['TenLoaiCongViec'];
                                                echo '<tr>
                                                    <th scope="row" style="text-align: center">' . $idtang . '</th>
                                                    <td style="text-align: center">' . $tenlcv . '</td>
                                                    <td style="text-align: center">
                                                        <button class = "btn btn-primary"><a href="sua_lcv.php?id=' . $row['id'] . '"><i class="fas fa-pencil-alt"></i></a></button>
                                                        <button class = "btn btn-danger" onclick="return Del();"><a href="xoa_lcv.php?id=' . $row['id'] . '" ><i class="fas fa-trash-alt"></i></a></button>
                                                    </td>
                                                </tr>';
                                            }
                                        }
