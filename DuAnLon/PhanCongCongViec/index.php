    ...
    <div class="wrapper">
        ...
        <div id="content">
            ...
            <div class="content-header">
                <div class="container-fluid">
                    <!-- lấy ra tên dự án và ngày bd và ngày kết thúc -->
                    <?php
                    $_ID = $_GET['id'];
                    ...
                    if ($conn != true) {
                        ...
                        $sql = "SELECT * FROM duanlon WHERE id = '$_ID' ";
                        ...
                        if ($result) {
                            while ($row = mysqli_fetch_assoc($result)) {
                                ...
                                $chuyennbd = date('d-m-Y', strtotime($ngaybd));
                                $ngaykt = $row['NgayKetThuc'];
                                $chuyennkt = date("d-m-Y", strtotime($ngaykt));
                                echo '<tr>
                                        <h4 style="font-family:Time New Roman" class="m-0"><strong>Quản Lý Công Việc Trong Dự Án "<span style="color: red;font-family:Time New Roman"">' . $tenda . '</span>" (<spam>từ ' . $chuyennbd . ' --> ' . $chuyennkt . '</spam>)</strong></h4>
                                    </tr>';
                            }
                        }
                    }
                    ...
                    <div class="col-lg-12">
                        <div class="card card-outline card-success" style="border-top: 3px solid rgb(48, 162, 48); border-radius: 5px;">
                            <div class="card-header">
                                ...
                                <div class="row">
                                    <form method="post">
                                        <input type="text" name="txtSearch" style="width: 450px; margin-left:10px" placeholder="Tìm Kiếm Theo Tên Công Việc">
                                    </form>
                                    <!-- lấy ra id của dự án trc đó để làm csdl -->
                                    <div class="card-tools" style="margin-left:280px">
                                        <?php
                                        $_ID = $_GET['id'];
                                        echo '<tr>
                                                <a class="btn btn-block btn-sm btn-default btn-flat border-primary" href="them_cvpc.php?idDuAn=' . $_ID . '"><i class="fa fa-plus"></i> Thêm Công Việc</a>
                                            </tr>';
                                        ?>
                                    </div>
                                </div>
                            ...
                            <table class="table-bordered">
                                <thead>
                                    <tr>
                                        ...
                                        <th scope="col" style="text-align: center">Người làm</th>
                                ...
                                <tbody>
                                    ...
                                    if ($conn != true) {
                                        ...
                                        // $sql = "SELECT * FROM `pc_congviec_dal` WHERE  idDuAn = '$_ID'";
                                        //----------------------------phân trang khi click--------------------------------
                                        $sql = mysqli_query($conn, "select * from `pc_congviec_dal` WHERE  idDuAn = '$_ID'");
                                        ...
                                        $sql = "SELECT * FROM `pc_congviec_dal` WHERE  idDuAn = '$_ID' LIMIT $start,$limit";

                                        //----------------------------------Tìm Kiếm + Phân Trang -----------------------------
                                        if (isset($_POST['txtSearch']) && $_POST['txtSearch'] != '') {
                                            $KeyWord = $_POST['txtSearch'];
                                            $sql = "SELECT * FROM `pc_congviec_dal` WHERE  idDuAn = '$_ID' and TenCongViec LIKE N'%" . $KeyWord . "%' ";
                                            //var_dump($sql);
                                        } else {
                                            // $sql = "select * from  `pc_congviec_dal` WHERE  idDuAn = '$_ID'";
                                            //------------------------------phân trang--------------------------------
                                        ...
                                        if ($result) {
                                            ...
                                            while ($row = mysqli_fetch_assoc($result)) {
                                                ...
                                                echo '<tr>
                                                      ...
                                                      <td style="text-align: center">' . $nguoilam . '</td>
                                                      <td style="text-align: center">' . $loaicv . '</td>
                                                      <td style="text-align: center">' . $chuyennbd . '</td>
                                                      <td style="text-align: center">' . $chuyennkt . '</td>
                                                      <td style="text-align: center">' . $tinhtrang . '</td>
                                                      <td style="text-align: center">
                                                            <button class = "btn btn-primary"><a href="sua_cvpc.php?id=' . $row['id'] . '"><i class="fas fa-pencil-alt"></i></a></button>
                                                            <button class = "btn btn-danger" onclick="return Del();"><a href="xoa_cvpc.php?id=' . $row['id'] . '" ><i class="fas fa-trash-alt"></i></a></button>
                                                      </td>
                                                    </tr>';
                                            }
                                        }
